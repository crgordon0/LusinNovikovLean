...
sections of `f` -/
def IDual₀ (f : α → β) : Set (Set α) := {A : Set α | BorelPartialSection f Aᶜ}
...
def I (f : α → β) : Set (Set α) := {s : Set α | sᶜ ∈ (IDual f).sets}

/-- Borel partial sections are contained in I -/
theorem borelpartialsection_mem_I (s : Set α) (hs : BorelPartialSection f s) :
  s ∈ I f := by
  rw[I, IDual, IDual₀]
...
theorem subset_mem_I {s t : Set α} (ssubt : s ⊆ t) (ht : t ∈ I f) :
  s ∈ I f := by
  rw[I] at ht ⊢
...
theorem union_mem_I {s t : Set α} (hs : s ∈ I f) (ht : t ∈ I f) :
  s ∪ t ∈ I f := by
...
theorem iUnion_mem_I (s : ℕ → Set α) :
...
theorem null_sing_iff_in_I (u : Set α) : IsNull f {u} ↔ u ∈ I f := by
  constructor
  . intro hF
    rw[IsNull] at hF
...
theorem nonempty_of_nonnull (Fnonnull : ¬IsNull f F) : ∀ s ∈ F,
  Set.Nonempty s := by
    intro s hs
